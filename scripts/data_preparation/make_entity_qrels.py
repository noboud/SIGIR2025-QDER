#!/usr/bin/env python3
"""
Create entity qrels from document qrels.
Converts document-level relevance judgments to entity-level judgments.
"""
...
def create_qrels(
    ...
    """Create entity qrels from document qrels and document-entity mappings."""
    data: List[str] = []
...
def create_qrels_file_strings(query_id: str, entities: Set[str], data: List[str], label: str) -> None:
    """Create TREC format qrels strings for entities."""
...
def read_docs(file: str) -> Dict[str, List[str]]:
    """Read document corpus with entity mappings."""
    docs: Dict[str, List[str]] = {}
    with open(file, 'r') as f:
        for line in tqdm(f):
            d = json.loads(line)
